...
for text, text_name in [(text1, 'text1'), (text2, 'text2')]:
    for pattern, pattern_name in [(existing_substring, 'existing_substring'), (non_existing_substring, 'non_existing_substring')]:
        results[(text_name, pattern_name, 'Boyer-Moore')] = measure_time(boyer_moore, text, pattern)
        results[(text_name, pattern_name, 'KMP')] = measure_time(kmp, text, pattern)
        results[(text_name, pattern_name, 'Rabin-Karp')] = measure_time(rabin_karp, text, pattern)

# Аналіз результатів
for key, value in results.items():
    print(f"Text: {key[0]}, Pattern: {key[1]}, Algorithm: {key[2]}, Time: {value} seconds")
...
for text_name in ['text1', 'text2']:
    for pattern_name in ['existing_substring', 'non_existing_substring']:
        best_algorithm = min([(algo, results[(text_name, pattern_name, algo)]) for algo in ['Boyer-Moore', 'KMP', 'Rabin-Karp']], key=lambda x: x[1])
        summary[(text_name, pattern_name)] = best_algorithm

for key, value in summary.items():
    print(f"Text: {key[0]}, Pattern: {key[1]}, Best Algorithm: {value[0]}, Time: {value[1]} seconds")
